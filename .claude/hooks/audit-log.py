...
# Patterns that strongly indicate a secret. Replaced with [REDACTED] before
# anything is written to docs/, since audit files are committed.
REDACT_PATTERNS = (
    # key=value / key: value forms (token, api_key, password, secret, etc.)
    re.compile(
        r"((?:api[-_]?key|access[-_]?token|auth[-_]?token|bearer[-_]?token|"
        r"refresh[-_]?token|secret|password|passwd|pwd|client[-_]?secret|"
        r"private[-_]?key)[\"']?\s*[:=]\s*[\"']?)"
        r"([^\s\"'&]+)",
        re.IGNORECASE,
    ),
    # HTTP Authorization headers: "Bearer <token>" / "Basic <token>"
    ...
    # Generic high-entropy hex blobs ≥ 32 chars (covers many tokens)
    re.compile(r"\b([A-Fa-f0-9]{32,})\b"),
    # JWT-looking strings (three base64url segments)
