...
### Changed
- **`/discovery` Step 2 is now a progressive multi-choice interview, not a batched gap-list.** Previously the command asked all unanswered topics in a single message and trained "fill these all at once" prose answers. The new flow runs ONE `AskUserQuestion` call per gap — 2-4 concrete inferred options per question, recommended default first, options sourced from codebase scan, scenario template prompts, prior topic answers, and Step 0 brain lessons. Each answer is recorded in the doc immediately and informs the option set of the next question. Free-text chat-style Q&A is explicitly forbidden inside `/discovery` (use `/brainstorm` if conversational free-text exploration is needed).
- **`/discovery` interview length is bounded by a Sufficiency Bar, not by the 10-topic count.** The interview ends only when the doc is detailed enough for `/requirement` to produce a concrete Implementation Plan — defined as all six of: every topic answered or confidently inferred · Problem Statement names who/what/why · ≥2 candidate approaches with real Pros/Cons · constraints the picked approach must respect are explicit · Scope Estimate decisively picks single-epic vs multi-epic with in-/out-of-scope drawn · remaining Open Questions are tagged `blocking-for-planning` or `carry-forward-to-/requirement`. If any item is still vague, the command runs a follow-up drill-down (another `AskUserQuestion` on the weakest topic) — no fixed cap on question count. The previous "Stopping early" rule (which stopped the interview as soon as remaining topics looked inferable) is removed.
- **Follow-up drill-downs within a topic are now first-class.** Step 2 adds a `Follow-up drill-downs` section with concrete examples ("users are slow" → which step, how slow, how often; "internal team" → which team, how many people, frequency of use) and a 1–4-question-per-topic guideline. A drill-down stops the moment that topic alone wouldn't hold back `/requirement` from writing its Implementation Plan.
- **`.claude/rules/discovery.md` rewritten to match.** The rule's frontmatter `description` now states the new behavior (progressive multi-choice interview, ask as many questions as needed including drill-downs). The Sufficiency Bar is repeated in the rule body so any future rewrite of `/discovery` inherits the stop condition. A `Relationship to clarification.md` section clarifies that `.claude/rules/clarification.md`'s "never one-by-one" still applies to `/requirement`/`/implement` clarification steps but does NOT apply to `/discovery` Step 2 — the two rules cover different phases and do not conflict.
- **`/brainstorm` "when to choose" block sharpened.** The contrast with `/discovery` now reads as free-text-conversational (brainstorm) vs structured-multi-choice (discovery) instead of "conversational vs fill-template-directly", since `/discovery` is no longer a fill-the-template-directly command.

### Rationale
- **The old batched-gap pattern trained minimal-effort answers.** Asking the user to write prose for every unanswered topic in one message produces "idk", "see above", and "we'll figure it out" — exactly the kind of vague input that leaks into the Approaches / Constraints / Scope sections and forces `/requirement` to re-litigate the same questions later. Progressive multi-choice inverts the cost: the AI commits to inferences (visible as the recommended option), the user confirms or overrides in one click, and each answer narrows the option space for the next question. Round-trip count is the same when input is already rich (most topics skip), but cognitive load drops sharply when input is thin — the user reacts to concrete tradeoffs instead of authoring prose from scratch.
- **A fixed question cap was the wrong primitive.** The previous "stop early when remaining topics look inferable" rule optimized for question count rather than spec quality. Real discoveries hit topics where one answer is still too coarse to feed `/requirement` (e.g. "use library X" without specifying integration mode); capping or skipping there pushes the unresolved gap downstream into `/requirement` or `/implement`, where it costs more to fix. The Sufficiency Bar replaces "fewer questions is better" with "ask whatever is needed so `/requirement` can produce an Implementation Plan without re-asking" — and explicitly does NOT require implementation-ready detail, since that's `/requirement`'s job. This keeps `/discovery` focused on context/scope/approach decisions while preventing the false-economy of stopping too early.
